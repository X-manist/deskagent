...
function resolveNativeRuntime() {
  ...
  const builtName = process.platform === 'win32' ? 'deskagent-core.exe' : 'deskagent-core';
  const builtPath = path.resolve(
    root, '..', 'core', 'codex', 'codex-rs', 'target', 'release', builtName
  );
  ...
  const triple = TRIPLE_BY_PLATFORM[key];
  const platPkg = PKG_BY_PLATFORM[key];
  if (!triple || !platPkg) return '';
  const pkgRoot = path.dirname(path.dirname(launcher));
  const nativeExe = process.platform === 'win32' ? 'codex.exe' : 'codex';
  const candidate = path.join(
    pkgRoot, 'node_modules', '@openai', platPkg, 'vendor', triple, 'bin', nativeExe
  );
...
function resolveOsToolsBinary() {
  const osToolsExe = process.platform === 'win32' ? 'deskagent-os-tools.exe' : 'deskagent-os-tools';
  ...
  const releaseBin = path.join(crateDir, 'target', 'release', osToolsExe);
...
if (osToolsPath) {
  const osToolsTarget = path.join(binDir, process.platform === 'win32' ? 'deskagent-os-tools.exe' : 'deskagent-os-tools');
  fs.copyFileSync(osToolsPath, osToolsTarget);
  if (process.platform !== 'win32') fs.chmodSync(osToolsTarget, 0o755);
  ...
  console.warn('[prepare] WARNING: Rust OS helper not found; desktop automation will use the Node fallback in development only.');
}
