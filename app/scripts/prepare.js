...
function writePackagedClientEnv() {
  fs.mkdirSync(path.dirname(envDest), { recursive: true });
  const lines = [
    '# Generated by app/scripts/prepare.js. Client-safe values only.',
    `DESKAGENT_BACKEND_URLS=${process.env.DESKAGENT_BACKEND_URLS || process.env.DESKAGENT_BACKEND_URL || DEFAULT_CLOUD_BACKEND_URLS.join(',')}`,
    'DESKAGENT_DIRECT_RELAY_FALLBACK=false',
  ];
  if (process.env.DESKAGENT_MCP_PROFILE) {
    lines.push(`DESKAGENT_MCP_PROFILE=${process.env.DESKAGENT_MCP_PROFILE}`);
  }
  fs.writeFileSync(envDest, lines.join('\n') + '\n');
  console.log(`[prepare] wrote client-safe env -> ${path.relative(root, envDest)}`);
}
writePackagedClientEnv();
